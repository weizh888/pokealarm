...
class TelegramAlarm(Alarm):
    ...
    _defaults = {  # No touchy!!! Edit alarms.json!
        ...
        "invasions": {
            ...
            "sticker_url": get_image_url("telegram/invasions/<grunt_id_3>.webp"),
        },
    }
    ...
    def __init__(self, mgr, settings):
        ...
        alert_defaults = {
            ...
            "venue": self.pop_type(settings, "venue", utils.parse_bool, False),
            "venue_notify": self.pop_type(
                settings, "venue_notify", utils.parse_bool, True
            ),
            "map": self.pop_type(settings, "map", utils.parse_bool, True),
            "map_notify": self.pop_type(
                settings, "map_notify", utils.parse_bool, False
            ),
            "max_attempts": self.pop_type(settings, "max_attempts", int, 3),
        ...
        self._stop_alert = self.create_alert_settings("stops", settings, alert_defaults)
        self._gym_alert = self.create_alert_settings("gyms", settings, alert_defaults)
        self._egg_alert = self.create_alert_settings("eggs", settings, alert_defaults)
        self._raid_alert = self.create_alert_settings("raids", settings, alert_defaults)
    ...
    def create_alert_settings(self, kind, settings, alert_defaults):
        ...
        try:
            use_display_sticker = (
                alert_defaults.pop("use_display_sticker")
                and TelegramAlarm._defaults[kind]["display_sticker_url"] is not None
            )
            display_sticker_url = TelegramAlarm._defaults[kind]["display_sticker_url"]
        except KeyError:
        ...
        alert = TelegramAlarm.Alert(
            ...
            sticker_url=Alarm.pop_type(
                ...
                "sticker_url",
                str,
                display_sticker_url if use_display_sticker else default["sticker_url"],
            ),
            sticker_notify=Alarm.pop_type(
                settings, "sticker_notify", utils.parse_bool, default["sticker_notify"]
            ),
            message=Alarm.pop_type(settings, "message", str, default["message"]),
            message_notify=Alarm.pop_type(
                settings, "message_notify", utils.parse_bool, default["message_notify"]
            ),
            venue=Alarm.pop_type(settings, "venue", utils.parse_bool, default["venue"]),
            venue_notify=Alarm.pop_type(
                settings, "venue_notify", utils.parse_bool, default["venue_notify"]
            ),
            map=Alarm.pop_type(settings, "map", utils.parse_bool, default["map"]),
            map_notify=Alarm.pop_type(
                settings, "map_notify", utils.parse_bool, default["map_notify"]
            ),
            max_attempts=Alarm.pop_type(
                settings, "max_attempts", int, default["max_attempts"]
            ),
            web_preview=Alarm.pop_type(
                settings, "web_preview", utils.parse_bool, default["web_preview"]
            ),
        )
    ...
    def startup_message(self):
        if self._startup_message:
            self.send_message(self._bot_token, self._chat_id, "PokeAlarm activated!")
